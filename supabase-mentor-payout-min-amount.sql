-- Ödeme talebi minimum tutarı ve kısmi çekim doğrulaması.

CREATE OR REPLACE FUNCTION public.get_mentor_payout_min_amount ()
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
...
REVOKE ALL ON FUNCTION public.get_mentor_payout_min_amount () FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_mentor_payout_min_amount () TO authenticated;
...
BEGIN
  ...
  v_min_amount := public.get_mentor_payout_min_amount();
  ...
  RETURN jsonb_build_object(
    ...
    'payout_min_amount', v_min_amount
  );
END;
$$;
...
-- get_mentor_wallet_summary: minimum tutar bilgisi
