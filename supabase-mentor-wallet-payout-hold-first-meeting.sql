...
-- supabase-mentor-wallet-payout-hold.sql ve supabase-mentor-wallet-held-balance-list.sql sonrasında çalıştırın.
...
COMMENT ON FUNCTION public.get_package_order_first_meeting_at (uuid) IS
  'Siparişe bağlı mentör-öğrenci-paket için onaylanmış en erken görüşme zamanı.';
...
COMMENT ON FUNCTION public.mentor_wallet_sale_withdrawable_at (uuid, timestamptz) IS
  'Paket satışı net tutarının çekilebilir olduğu zaman. İlk görüşme yoksa NULL.';
...
REVOKE ALL ON FUNCTION public.package_order_payout_hold_reason (uuid, timestamptz) FROM PUBLIC;
