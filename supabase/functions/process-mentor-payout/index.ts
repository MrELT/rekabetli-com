...
async function ensureRecipient(
  ...
  const { error } = await admin.rpc("save_mentor_wise_recipient", {
  ...
  if (error) {
    throw new Error(`save_mentor_wise_recipient:${error.message}`);
  }

  return recipientId;
}

async function tryIssueSelfBilledInvoice(
  admin: ReturnType<typeof createServiceClient>,
  requestId: string,
  ...
  try {
    ...
    return {
      ...
      self_billed_invoice_path: issued.storagePath,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("self-billed-invoice:", message);
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: beginError } = await admin.rpc("begin_mentor_payout_processing", {
      p_request_id: requestId,
    });
    ...
    const sourceCurrency = (Deno.env.get("WISE_SOURCE_CURRENCY") || "GBP").toUpperCase();
    const targetAmount = Number(transferDetails.amount_net);

    if (!Number.isFinite(targetAmount) || targetAmount <= 0) {
      await admin.rpc("fail_mentor_payout", {
        p_request_id: requestId,
    ...
    if (isWiseMockEnabled()) {
      const quoteUuid = String(transferDetails.wise_quote_id || `mock-${requestId}`);
      const transferId = `mock-${Date.now()}`;
      const { data: completed, error: completeError } = await admin.rpc("complete_mentor_payout", {
        p_request_id: requestId,
        p_wise_transfer_id: transferId,
        p_wise_quote_id: quoteUuid,
      });

      if (completeError) {
        console.error("complete_mentor_payout:", completeError.message);
        return jsonResponse(
          {
            error: "payout_complete_failed",
      ...
      const invoice = await tryIssueSelfBilledInvoice(admin, requestId);
    ...
    const existingQuoteId = String(transferDetails.wise_quote_id || "").trim();
    let quoteUuid = existingQuoteId;
    if (!quoteUuid) {
    ...
    const transfer = await wiseRequest("POST", "/v1/transfers", {
      ...
      details: {
        reference: "rekabetli mentor odeme",
      },
    });
    ...
    const { data: completed, error: completeError } = await admin.rpc("complete_mentor_payout", {
      p_request_id: requestId,
      p_wise_transfer_id: String(transferId),
      p_wise_quote_id: quoteUuid,
    });
    ...
    console.error("process-mentor-payout:", message);
