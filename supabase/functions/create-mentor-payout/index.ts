...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      await assertApiRateLimit(adminClient, `mentor_payout:${authData.user.id}`, 5, 3600);
    } catch (rateError) {
    ...
    if (body.selfBillingAgreed !== true && body.self_billing_agreed !== true) {
      return jsonResponse({ error: "self_billing_agreement_required" }, 400);
    }
    ...
    const { data, error } = await adminClient.rpc("request_mentor_payout", {
      p_mentor_id: authData.user.id,
      p_amount: split.amountRequested,
      p_self_billing_agreed: true,
    ...
    console.error("create-mentor-payout:", message);
